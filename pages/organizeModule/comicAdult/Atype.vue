<template>
  <b-container fluid class="page-min-height py-3">
    <UiPageTitle title="A형 편성모듈 관리" :breadcrumb="breadcrumb">
      <template #buttons>
        <b-button variant="primary" to="comicAdult/AtypeAdd">
          등록
        </b-button>
      </template>
    </UiPageTitle>
    ...
    <b-row class="pt-5">
      <b-col align-self="center">
        <h6>A형 편성모듈 목록 ({{ totalRows ? totalRows : '0' }})</h6>
      </b-col>
    </b-row>
    <hr class="my-2">
    ...
    >
      ...
      <template #cell(setting)="data">
        <b-button
          ...
          @click="removeAtype(data.item.id)"
        >
          삭제
        </b-button>
      </template>
  ...
  computed: {
    breadcrumb () {
      const arr = [
        { label: '홈', url: '/' },
        { label: '편성모듈', url: '#' },
        { label: 'A형 편성모듈 관리', url: this.$route.path }
      ]
      return arr
    }
  },
