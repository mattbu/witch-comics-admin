<template>
  <b-container fluid class="page-min-height py-3">
    <UiPageTitle title="소설 홈 관리" :breadcrumb="breadcrumb">
      <template #buttons>
        <b-button variant="primary">
          미리보기
        </b-button>
        <b-button variant="primary" to="/organizeModule/add?type=novel">
          등록
        </b-button>
    ...
    <b-row>
      <b-col cols="12">
        <b-button
          :variant="moduleType === 2 ? 'primary' : 'outline-primary'"
          @click="
            moduleType = 2
            $router.push('/organizeModule/novelHome?type=all')
          "
        >
          일반
        </b-button>
        <b-button
          :variant="moduleType === 4 ? 'primary' : 'outline-primary'"
          @click="
            moduleType = 4
            $router.push('/organizeModule/novelHome?type=adult')
          "
        >
          성인
        </b-button>
  ...
  computed: {
    breadcrumb () {
      const arr = [
        { label: '홈', url: '/' },
        { label: '편성모듈 관리', url: '#' },
        { label: '소설 홈 관리', url: this.$route.fullPath }
      ]
      return arr
    }
  },
    ...
    setLinkUrl (type, id) {
      if (type === '이따무') {
        if (this.moduleType === 2) {
          return `/organizeModule/${id}/itamuAdd?type=novel&rate=all`
        }
        return `/organizeModule/${id}/itamuAdd?type=novel&rate=adult`
      } else if (type === '관리자') {
        return `/organizeModule/${id}?type=novel`
      } else if (type === '카드형') {
        if (this.moduleType === 2) {
          return `/organizeModule/${id}/cardAdd?type=novel&rate=all`
        }
        return `/organizeModule/${id}/cardAdd?type=novel&rate=adult`
      } else if (type === '동영상') {
        if (this.moduleType === 2) {
          return `/organizeModule/${id}/videoAdd?type=novel&rate=all`
        }
        return `/organizeModule/${id}/videoAdd?type=novel&rate=adult`
      } else if (type === '매거진') {
        return `/organizeModule/novelHome/${id}/magazineAdd`
      } else if (type === '이벤트') {
        if (this.moduleType === 2) {
          return `/organizeModule/${id}/eventAdd?type=novel&rate=all`
        }
        return `/organizeModule/${id}/eventAdd?type=novel&rate=adult`
      } else if (type === '오늘UP') {
        return '/organizeModule/novelWeek'
      } else if (type === '실시간랭킹') {
        return '/organizeModule/novelRank'
      }
      return '/organizeModule/novelHome'
    },
    async changeOrder () {
      try {
