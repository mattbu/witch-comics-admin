<template>
  <b-container fluid class="page-min-height py-3">
    <UiPageTitle title="웹툰/만화 요일 관리" :breadcrumb="breadcrumb">
      <template #buttons>
...
export default {
  ...
  computed: {
    breadcrumb () {
      const arr = [
        { label: '홈', url: '/' },
        { label: '편성모듈 관리', url: '#' },
        { label: '웹툰/만화 요일 관리', url: this.$route.fullPath }
      ]
      return arr
    }
  },
  ...
  methods: {
    ...
    async getList () {
      this.isLoading = true
      try {
        const { status, data: { data } } = await this.$axios.get(`/api/manager/series/list/${this.currentDay}/1`, {
          params: {
    ...
    async getWeekday () {
      try {
        const { data: { data } } = await this.$axios.get('/api/manager/series/tab/list/1', {
          params: {
            title: this.filter.title
          }
        })
