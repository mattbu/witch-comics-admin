<template>
  <b-container fluid class="page-min-height py-3">
    <UiPageTitle title="상품 그룹 수정" :breadcrumb="breadcrumb">
      <template #buttons>
        <b-button variant="primary" @click="editGroup">
          수정
        </b-button>
      </template>
    </UiPageTitle>
    ...
    >
      ...
      <template #cell(setting)="data">
        ...
        <b-button v-else variant="outline-secondary" class="table-inner-btn" @click="removeRow(data.item.id)">
          삭제
        </b-button>
      </template>
    </b-table>
...
export default {
  ...
  methods: {
    ...
    async editGroup () {
      try {
        if (this.checkValidation()) {
          ...
          const res = await this.$axios.put(`/api/manager/product/group/${this.$route.params.id}`, this.form)

          if (res.status === 200) {
            alert('수정이 완료 되었습니다.')
            await this.$router.push('/product/general')
          }
        }
      } catch (e) {
        console.log(e)
    ...
    async getList () {
      ...
      try {
        ...
        if (status === 200) {
          ...
          this.form = data
          this.productList = [this.productList[0], ...data.productlists]
        }
      } catch (e) {
        console.log(e)
      } finally {
    ...
    async addRow () {
      try {
        const res = await this.$axios.post(`/api/manager/product/product/${this.$route.params.id}`, {
      ...
      this.productList[0].price = ''
      this.productList[0].coin = ''
      this.productList[0].showPrice = ''
      this.productList[0].discountRate = ''
      this.productList[0].bonusCoin = ''
      this.productList[0].bonusPoint = ''
    },
    async removeRow (id) {
      try {
        const res = await this.$axios.delete(`/api/manager/product/product/${id}`)
