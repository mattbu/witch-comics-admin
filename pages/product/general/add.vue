  ...
  <b-container fluid class="page-min-height py-3">
    ...
    >
      ...
      <template #cell(showPrice)="data">
        ...
        <template v-else>
          {{ productList[data.index].showPrice }}
        </template>
      </template>
      <template #cell(discountRate)="data">
        ...
        <template v-else>
          {{ productList[data.index].discountRate }}
        </template>
      </template>
      <template #cell(bonusCoin)="data">
        ...
        <template v-else>
          {{ productList[data.index].bonusCoin }}
        </template>
      </template>
      <template #cell(bonusPoint)="data">
        ...
        <template v-else>
          {{ productList[data.index].bonusPoint }}
        </template>
      </template>
...
export default {
  ...
  computed: {
    breadcrumb () {
      const arr = [
        { label: '홈', url: '/' },
        { label: '프로모션 관리', url: '#' },
        { label: '상품 그룹 등록', url: '/product/general/add' }
      ]
      return arr
    }
  },
  ...
  methods: {
    checkValidation () {
      if (!this.form.title) {
        alert('그룹 타이틀을 입력해 주세요.')
        return false
      } else if (!this.start.dt) {
        alert('시작일을 입력해 주세요.')
        return false
      } return true
    },
    setForm () {
      if (this.productList.length > 1) {
        this.form.productList = this.productList.filter((row, idx) => idx !== 0)
      }
    ...
    async addProductGroup () {
      ...
      try {
        if (this.checkValidation()) {
          const res = await this.$axios.post('/api/manager/product/group', this.form)
