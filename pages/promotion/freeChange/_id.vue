<template>
  <b-container fluid class="page-min-height py-3">
    <UiPageTitle title="무료전환 상세" :breadcrumb="breadcrumb">
      <template #buttons>
        <b-button
        ...
        <table class="table border mb-0 table-layout-fixed">
          ...
          <tbody class="text-center">
            ...
            <tr>
              <th>작품명</th>
              <td colspan="3">
                {{ freeChangeInfo.title ? freeChangeInfo.title : '-' }}
              </td>
              <th>작품 아이디</th>
              <td colspan="3">
                {{ freeChangeInfo.id ? freeChangeInfo.id : '-' }}
              </td>
              <th>작가명</th>
              <td colspan="3">
                {{ freeChangeInfo.writer ? freeChangeInfo.writer : '-' }}
              </td>
            </tr>
            <tr>
              <th>유형</th>
              <td colspan="3">
                {{ freeChangeInfo.isBook ? freeChangeInfo.isBook : '-' }}
              </td>
              <th>구분</th>
              <td colspan="3">
                {{ freeChangeInfo.menucode === '1' ? '웹툰/만화' : freeChangeInfo.menucode === '2' ? '소설' : '-' }}
              </td>
              <th>상태</th>
              <td colspan="3">
                {{ freeChangeInfo.status ? freeChangeInfo.status : '-' }}
              </td>
            </tr>
            <tr>
              <th>CP사</th>
              <td colspan="3">
                {{ freeChangeInfo.cpName ? freeChangeInfo.cpName : '-' }}
              </td>
              <th>장르</th>
              <td colspan="3">
                {{ freeChangeInfo.genre ? freeChangeInfo.genre : '-' }}
              </td>
              <th>연령등급</th>
              <td colspan="3">
                {{ freeChangeInfo.rate ? freeChangeInfo.rate : '-' }}
              </td>
            </tr>
            <tr>
              <th>연재일</th>
              <td colspan="3">
                {{
                  freeChangeInfo.displayStartDate
                    ? freeChangeInfo.displayStartDate
                    : '-'
                }}
              </td>
              <th>가격</th>
              <td colspan="3">
                {{ freeChangeInfo.price ? `물약 ${freeChangeInfo.price}개` : '-' }}
              </td>
              <th>무료회차수</th>
              <td colspan="3">
                {{ freeChangeInfo.freeCnt ? `${freeChangeInfo.freeCnt}화` : '-' }}
              </td>
            </tr>
          </tbody>
        </table>
            ...
            <tr>
              ...
              <td colspan="3">
                <div class="d-inline-flex">
                  <b-form-input v-model="form.waitTime" class="w-30" />
                  <span class="align-self-center ml-2">시간</span>
                </div>
              </td>
            </tr>
            <th>시작일</th>
  ...
  computed: {
    breadcrumb () {
      const arr = [
        { label: '홈', url: '/' },
        { label: '프로모션 관리', url: '#' },
        { label: '무료전환 상세', url: this.$route.path }
      ]
      return arr
    }
  },
