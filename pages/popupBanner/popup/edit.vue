<template>
    <b-container fluid class="page-min-height py-3">
      <UiPageTitle title="팝업 수정" :breadcrumb="breadcrumb">
        <template #buttons>
          <b-button variant="primary" @click="editPopup">
            수정
          </b-button>
        </template>
      </UiPageTitle>
  ...
  computed: {
    breadcrumb () {
      const arr = [
        { label: '홈', url: '/' },
        { label: '팝업/배너 관리', url: '#' },
        { label: '팝업 수정', url: this.$route.path }
      ]
      return arr
    }
  },
  ...
  methods: {
    ...
    async editPopup () {
      try {
        ...
        {
          title: this.popupInfo.name
        }
        )
        if (status === 200) {
          alert('팝업수정이 완료 되었습니다.')
        }
      } catch (err) {
