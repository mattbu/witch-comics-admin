<template>
  <b-container fluid class="page-min-height py-3">
    ...
    >
      <template #buttons>
        <b-button variant="primary" to="/board/notice/add">
          공지사항 등록
        </b-button>
      </template>
    </UiPageTitle>
    ...
    >
      ...
      <template #cell(setting)="data">
        <b-button variant="outline-secondary" class="table-inner-btn" :to="`/board/notice/${data.item.id}`">
          보기
        </b-button>
      </template>
    </b-table>
  ...
  computed: {
    breadcrumb () {
      const arr = [
        { label: '홈', url: '/' },
        { label: '게시판 관리', url: '#' },
        { label: '공지사항', url: this.$route.path }
      ]
      return arr
    }
  },
