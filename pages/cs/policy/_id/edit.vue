<template>
  <b-container fluid class="page-min-height py-3">
    <UiPageTitle title="정책 수정" :breadcrumb="breadcrumb">
      <template #buttons>
        <b-button variant="primary" @click="editPolicy">
          수정
        </b-button>
      </template>
    </UiPageTitle>
  ...
  computed: {
    breadcrumb () {
      const arr = [
        { label: '홈', url: '/' },
        { label: '서비스 운영 관리', url: '#' },
        { label: '정책 수정', url: this.$route.path }
      ]
      return arr
    }
  },
    ...
    async editPolicy () {
      try {
        const { status } = await this.$axios.put(
          `/api/manager/policy/${this.currentId}`,
          this.form
          // {
    ...
    async detailList () {
      try {
        const {
          ...
          `/api/manager/policy/${this.$route.params.id}`
        )
        if (status === 200) {
          this.form = data
        }
