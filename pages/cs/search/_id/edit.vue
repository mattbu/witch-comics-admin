<template>
  <b-container fluid class="page-min-height py-3">
    <UiPageTitle title="검색어 수정" :breadcrumb="breadcrumb">
      <template #buttons>
        <b-button variant="primary" @click="editSearch">
          수정
        </b-button>
      </template>
    </UiPageTitle>
            ...
            <tr>
              ...
              <td colspan="3">
                <b-form-input
                  ...
                  type="text"
                  class="w-50"
                  placeholder="관리자구분 제목을 입력해주세요. 사용자 페이지에는 노출 되지 않습니다."
                />
              </td>
            </tr>
            <tr>
...
export default {
  ...
  computed: {
    breadcrumb () {
      const arr = [
        { label: '홈', url: '/' },
        { label: '서비스 운영 관리', url: '#' },
        { label: '검색어 등록', url: this.$route.path }
      ]
      return arr
    }
  },
  ...
  methods: {
    async editSearch () {
      try {
        const { status } = await this.$axios.put(`//${this.currentId}`, {
    ...
    async detailList () {
      try {
        const {
          status,
          data: { data }
        } = await this.$axios.get(`/${this.$route.params.id}`)
