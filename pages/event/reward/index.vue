  ...
  <b-container fluid class="page-min-height py-3">
    ...
    >
      ...
      <template #cell(setting)="data">
        <b-button variant="outline-secondary" class="table-inner-btn" :to="`/event/reward/${data.item.id}`">
          상세
        </b-button>
      </template>
    </b-table>
  ...
  data () {
    return {
      ...
      fields: [
        ...
        { key: 'name', label: '작품명' },
        { key: 'type', label: '콘텐츠' },
        { key: 'period', label: '리워드 기간' },
        { key: 'rewardSetting', label: '리워드 설정' },
        { key: 'rewardQuantity', label: '갯수' },
        { key: 'buySetting', label: '구매화 설정' },
  ...
  computed: {
    breadcrumb () {
      const arr = [
        ...
        { label: '리워드 지급 내역', url: this.$route.fullPath }
      ]
      return arr
    }
  },
