      ...
      <b-col lg="12" class="my-2">
        ...
        >
          ...
          <template #cell(check)="data">
            <b-form-checkbox v-model="selectedItems" :value="data.item" @click="removeItem(data.item)" />
          </template>
  ...
  methods: {
    ...
    async getList () {
      ...
      try {
        const {
          data: { data }
        } = await this.$axios.get(`/api/manager/schedule/acartoon/search/${this.menuCode}`, {
          params: {
            title: this.keyword
          }
        })
